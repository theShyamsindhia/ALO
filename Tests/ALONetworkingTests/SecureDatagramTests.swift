...
struct NetworkFixture {
    ...
    static func transcript(room: UUID = room, initiator: UUID = receiver, responder: UUID = sender,
                           nonce: UInt8 = 3, capabilities: PeerCapabilities = .mobile,
                           wireVersions: [UInt16] = [2]) throws -> AdmissionTranscript {
        try .init(roomID: room, initiatorID: initiator, responderID: responder, connectionID: session,
            initiatorKeyHash: Data(repeating: 1, count: 32), responderKeyHash: Data(repeating: 2, count: 32),
            initiatorNonce: Data(repeating: nonce, count: 32), responderNonce: Data(repeating: 4, count: 32),
            initiatorOffer: ProtocolOffer(wireVersions: wireVersions, stateSyncVersions: [1], capabilities: capabilities),
            responderOffer: ProtocolOffer(wireVersions: [2], stateSyncVersions: [1], capabilities: .desktop), policy: .secureV2)
    }
}

@Suite("Secure v2 datagrams")
