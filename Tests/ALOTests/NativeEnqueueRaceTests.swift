...
/// Injects a real elapsed scheduling delay, not actual engine-lock contention.
/// PCM and advancing player sample time are native offline output, not acoustic evidence.
@Suite(.serialized) @MainActor
struct NativeEnqueueRaceTests {
    @Test(arguments: [Float(0.99), 1, 1.01], [false, true])
    func nativeEnqueueCannotSilentlyMissAdmittedSourcePosition(rate: Float, delayed: Bool) async throws {
        ...
        let player = try SynchronizedPlayer(audioOutput: output, liveDJAudio: DJLiveAudio(), beforeNativeSchedule: {
            ...
            hookElapsed = MonotonicClock.nowNanos() - began
        })
        ...
        for block in 0..<20 {
            ...
            if absoluteMarker == nil,
               let index = (0..<240).first(where: { scratch.floatChannelData![0][$0] > 0.25 }) {
                absoluteMarker = (delayed ? 0 : 120) + block * 240 + index
            }
        }
        ...
        print("NATIVE_ENQUEUE_RACE rate=\(rate) delayed=\(delayed) beforeSample=\(before) afterSample=\(after) hookMs=\(Double(hookElapsed)/1_000_000) marker=\(marker) expected=\(expected) displacementMs=\((Double(marker)-expected)/48) late=\(player.syncReport().latePacketCount) resync=\(player.syncReport().resyncCount)")
