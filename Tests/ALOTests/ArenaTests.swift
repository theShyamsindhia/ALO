...
struct ArenaRoomRosterTests {
    ...
    @Test @MainActor func receivedResultsCarryStableIDsAndDeduplicate() throws {
        ...
        // A delayed/coalesced timer produces one callback, not a guaranteed
        // three. Final results must not wait for the periodic state cadence.
        let now = ProcessInfo.processInfo.systemUptime
        host.update(at: now + 0.120); bus.drain()
        ...
        for tick in 2...6 { host.update(at: now + Double(tick) * 0.120); bus.drain() }
