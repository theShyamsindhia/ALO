...
struct RenderObservationTests {
    @Test func contentRecoveryReasonsAreDistinctCumulativeAndExplicitlyUnavailable() throws {
        ...
        #expect(counts.admissionDropped == 1)
        #expect(counts.lastReason == .contentAdmissionDropped)
        var recorder = RenderObservationRecorder()
        recorder.record(.init(observedAtNanos: 1, contentRecovery: counts))
        #expect(try #require(recorder.snapshot(at: 1)).detail.contains("content recoveries concealment=1, native-position=1, enqueue-window=1, admission-dropped=1, last=content-admission-dropped"))
