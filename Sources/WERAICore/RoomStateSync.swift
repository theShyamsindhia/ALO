        ...
        try withLock {
            ...
            do {
                ...
                guard candidate.heads() != document.heads() else {
                    // Ordered per-link TCP sync carries causally complete
                    // changes. Do not retain Automerge's in-memory pending
                    // dependency queue for duplicate/ack-only messages.
