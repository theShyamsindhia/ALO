...
public final class MacDeviceMessageReceiver: @unchecked Sendable {
    ...
    public func dispatchStored(connection: Connection, grantID: UUID, messageID: UUID) {
        guard connection.owner == issuer,
              let permit = admission.acquire(.init(grant: grantID, message: messageID)) else { return }
        queue.async { [weak self] in
            guard let self, !self.stopped, self.admission.isOpen, let session = self.sessions[connection.transport] else { return }
    ...
    private func handle(_ transport: UUID, _ value: NetworkDeviceTextTransport.Event) {
        ...
        case .messageAccepted(let session, let message):
            ...
            dispatchStored(connection: connection, grantID: message.grantID, messageID: message.messageID)
        case .closed:
...
final class ReceiverDispatchAdmission {
    ...
    func acquire(_ key: Key) -> ReceiverDispatchPermit? {
        lock.lock(); defer { lock.unlock() }
        guard !stopped, active.count < 32, active[key] == nil else { return nil }
        let token = UUID(); active[key] = token
        return ReceiverDispatchPermit(owner: self, key: key, token: token)
    }
