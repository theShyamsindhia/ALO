        ...
        VStack(alignment: .leading, spacing: 18) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    step("5 · Mix and share", "Use channel gain and EQ to balance the decks, then blend with the crossfader. Match tempo uses your entered BPMs; it does not detect beats or align their phase. Watch the master meter and lower gain at peaks. Share DJ mix sends both decks and pads to the channel. Stop the current broadcast first.")
                    step("Live broadcast on Deck A", "Select Live broadcast above A to process channel audio directly. The waveform and rewind slider use the last 32 seconds in memory. Jump live returns to incoming audio. Set BPM and loop the last selected beats, or use In/Out. A’s play key mutes/unmutes the input. EQ, gain and crossfader affect it; B and pads mix alongside it. At the broadcaster this changes the channel audio; at a listener it changes only this Mac. Voice chat bypasses these effects. Live tempo follows the stream. Channel Song retains the source’s supported previous/play/next controls.")
                    step("Stopping", "Stop all silences the decks and pads, including live input. Closing DJ Studio discards live history and restores normal channel audio; it also ends a dedicated DJ mix broadcast. Changing audio output stops playback; press Play again after the change. This version uses your Mac’s current output, without a separate headphone cue output.")
                }
            }
