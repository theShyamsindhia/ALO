...
Upstream playback-progress metadata carries optional duration and elapsed time. Timed results select the current line from the shared playback position; untimed results show the first available line in the player and the full text on demand. Track receipt time alone is not used as a lyric clock.
